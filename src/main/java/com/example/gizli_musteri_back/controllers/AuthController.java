...
import com.example.gizli_musteri_back.payload.request.SigninRequest;
import com.example.gizli_musteri_back.payload.request.SignupRequest;
import com.example.gizli_musteri_back.payload.response.MessageResponse;
import com.example.gizli_musteri_back.repositories.RoleRepository;
import com.example.gizli_musteri_back.repositories.UserRepository;
import com.example.gizli_musteri_back.requests.ResendCodeRequest;
import com.example.gizli_musteri_back.requests.VerifyCodeRequest;
import com.example.gizli_musteri_back.security.jwt.JwtUtils;
import com.example.gizli_musteri_back.security.services.UserDetailsImpl;
import com.example.gizli_musteri_back.security.services.UserDetailsServiceImpl;
import com.example.gizli_musteri_back.services.EmailService;
import com.example.gizli_musteri_back.services.TokenBlackListService;
import com.example.gizli_musteri_back.services.VerificationService;
    ...
    public ResponseEntity<?> verifyCode(@RequestBody VerifyCodeRequest verifyCodeRequest, @RequestHeader("User-Agent") String userAgent, HttpServletResponse response) {
     ...
     // Create JWT token (with device information)
        ...
        if ("prod".equals(env)) {
            ...
            cookie = String.format("jwt=%s; HttpOnly; Secure; SameSite= Lax; Path=/; Max-Age=%d", jwt, 4 * 60 * 60);
        } else {
